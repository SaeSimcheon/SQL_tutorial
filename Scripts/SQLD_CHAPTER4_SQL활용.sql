...
SELECT sum(td.DEPT_NO)  FROM TB_DEPT_21 td WHERE DEPT_NM = 'a' ;
...
SELECT NVL(1,1) FROM DUAL WHERE 1=2; 
...
SELECT COUNT(*),COUNT(A.NUM),SUM(A.NUM),MIN(A.NUM),AVG(A.NUM),MAX(A.NUM) FROM A;

SELECT EMP_NM ,EMP_NO ,TD.DEPT_CD  FROM TB_EMP_23 TE, TB_DEPT_23 TD WHERE TE.DEPT_CD = TD.DEPT_CD;



SELECT * FROM SQLD_DATA.TB_CUST_ORD ;

SELECT CUST_NO ,SALE_DE ,SALE_AMT,SUM(SALE_AMT)OVER(PARTITION BY CUST_NO)  FROM SQLD_DATA.TB_CUST_ORD ;

SELECT CUST_NO ,SALE_DE ,SALE_AMT,SUM(SALE_AMT)OVER(PARTITION BY CUST_NO ORDER BY SALE_DE)  FROM SQLD_DATA.TB_CUST_ORD ;
